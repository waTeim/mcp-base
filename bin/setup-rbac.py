...
class RBACManager:
    """Manages RBAC resources for MCP Base MCP server."""
...
def setup_rbac(
    ...
    """
    Set up RBAC resources for MCP Base MCP server.
    ...
    """
    print("\n" + "="*60)
    print("MCP Base MCP Server - RBAC Setup")
    print("="*60)
    print(f"\nConfiguration:")
...
def teardown_rbac(
    ...
    """
    Remove RBAC resources for MCP Base MCP server.
    ...
    """
    print("\n" + "="*60)
    print("MCP Base MCP Server - RBAC Teardown")
    print("="*60)
    print(f"\nConfiguration:")
    print(f"  Namespace: {namespace}")
    print(f"  Service Account: {service_account}")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="Setup or teardown RBAC resources for MCP Base MCP server",
...
Examples:
  ...
  python setup_rbac.py --namespace production --service-account mcp_base-server

  # Setup with namespace-scoped permissions
  python setup_rbac.py --namespace production --service-account mcp_base --scope namespace
...
Requirements:
  ...
  - For namespace-scoped: need admin permissions in the namespace
    ...
    parser.add_argument(
        "--service-account",
        default="mcp_base-server",
        help="Name of the service account (default: mcp_base-server)"
    )
