#!/usr/bin/env python3
"""
Create Kubernetes Secrets for MCP Base MCP Server
...
class KubernetesSecretCreator:
    ...
    def load_config(self, config_file: str) -> Dict[str, Any]:
        ...
        if not os.path.exists(config_file):
            ...
            print("  python bin/setup-auth0.py")
            sys.exit(1)
        ...
        try:
            namespace = client.V1Namespace(
                metadata=client.V1ObjectMeta(
                    ...
                    labels={
                        "name": self.namespace,
                        "created-by": "mcp_base-setup-script"
                    }
                )
            )
    ...
    def create_secret(
        ...
        final_labels = {
            "app": "mcp_base-server",
            "managed-by": "mcp_base-setup-script"
        }
        if labels:
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Create all Kubernetes secrets for MCP Base MCP Server deployment",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Examples:
  # Create all secrets
  python create_secrets.py --namespace default --release-name my-release

  # Dry run
  python create_secrets.py --namespace default --release-name my-release --dry-run

  # Force replace existing secrets
  python create_secrets.py --namespace default --release-name my-release --force
...
Secrets Created:
  ...
  2. <release-name>-jwt-signing-key
    ...
    print("=" * 70)
    print("🔐 Kubernetes Secret Creator for MCP Base MCP Server")
    print("=" * 70)
    print()
