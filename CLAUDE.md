...
| Script | Purpose |
|--------|---------|
| `sync-config.py` | Regenerate `make.env` from `mcp-project.yaml` (canonical config) |
| `configure-make.py` | Compatibility wrapper that delegates to `sync-config.py` |
| `smoke_test.py` | Build-time in-process FastMCP smoke test (catches contract violations before deploy) |

> The canonical project config (`mcp-project.yaml` at repo root) is the
> single source of truth for image names, ports, helm release, and
> namespace. `sync-config.py` reads it and writes `make.env`. Never
> hand-edit `make.env`.
...
```
mcp-base/
├── mcp-project.yaml            # CANONICAL config — ports, image names, helm release
├── mcp-base.yaml               # Release values overlay (helm -f target)
├── make.env                    # Generated by bin/sync-config.py — do NOT edit
├── Makefile                    # Build / test / coverage / helm targets
├── src/
│   ├── mcp_base_server.py     # Main MCP server (production / OAuth)
│   ├── mcp_base_test_server.py # No-auth test server (sidecar entrypoint)
│   └── mcp_base_tools.py      # Tools, resources, scaffold pipeline
├── bin/
│   ├── sync-config.py         # mcp-project.yaml → make.env
│   └── configure-make.py      # Compat wrapper around sync-config.py
├── templates/                  # Jinja2 templates that ship in scaffolds
│   ├── mcp-project.yaml.j2    # Canonical config template
│   ├── release-values.yaml.j2 # Per-release values overlay (rendered to <release>.yaml)
│   ├── server/                # Server code templates
│   ├── container/             # Dockerfile + Dockerfile.test
│   ├── helm/                  # Helm chart templates
│   ├── test/                  # Test framework + coverage harness templates
│   └── bin/                   # sync-config.py + configure-make.py + smoke_test.py
├── test/
│   ├── Dockerfile             # Test sidecar image (FROM main image)
│   ├── test-mcp.py            # Plugin runner (--port-forward, --no-auth, --token-file)
│   ├── run-coverage.py        # Coverage orchestrator
│   ├── requirements.txt       # coverage, httpx
│   └── plugins/               # Plugin tests (one per tool)
├── chart/                      # Helm chart for mcp-base itself
├── patterns/                   # Pattern documentation (exposed as pattern:// resources)
└── docs/                       # Reference documentation
```

## Two workflows

This repo supports two distinct workflows; see `README.md` for the
narrative version and [`docs/workflows.md`](docs/workflows.md) for the
deeper reference (dataflow diagrams, smoke-test snippets, cheatsheet).

- **Workflow A** — using mcp-base to generate a new MCP server. Agent
  calls `generate_server_scaffold`, retrieves files via `resources/read`,
  customizes `src/<server>_tools.py`, writes plugin tests, deploys.
- **Workflow B** — working on mcp-base itself. Edit `src/mcp_base_tools.py`
  (tools + role registry) or `templates/` (what ships in every scaffold).
  Run `make dev-coverage` to verify; smoke a generated scaffold to verify
  templates render to valid Python / valid YAML.
...
# Production server (auth-enforcing, /mcp):
python src/mcp_base_server.py --port 4200

# No-auth test server (for local testing without OIDC, /test):
python src/mcp_base_test_server.py --no-auth --port 4201

# Or via Makefile (uses ports from mcp-project.yaml):
make dev-coverage              # spawn test server under coverage and run tests
make config                    # regenerate make.env from mcp-project.yaml

# Build and run container
make build
docker run -p 4200:4200 wateim/mcp-base-server:latest
```
...
- `port`: HTTP port (default: 4200; test sidecar listens on port+1=4201)
...
Generated servers ship two server entry points and the chart wires both:

1. **Production server** (port `MCP_PORT`, default 4200, endpoint `/mcp`):
   FastMCP OAuth proxy / KeycloakAuthProvider — issues or accepts JWTs per
   the chosen `auth_type`. Reachable via Ingress.
2. **Test sidecar** (port `MCP_TEST_PORT`, default 4201, endpoint `/test`):
   no-auth. `NoAuthMiddleware` injects a mock `sub` / `iss` so tools that
   read `MCPContext.user_id` keep working. ClusterIP-only — never via
   Ingress.

The chart's `testSidecar` block enables/disables the second container; the
test image is auto-derived from the main image via
`s/-server$/-test-server/` so the sidecar runs the test image (which has
the `*_test_server.py` entrypoint), not the main image.

For local testing without OIDC setup, run the test server directly:

```bash
make dev-run-test                        # spawns the no-auth test server on $(MCP_TEST_PORT)
make test                                # runs plugin suite against http://localhost:$(MCP_TEST_PORT)/test

# In-cluster against the deployed sidecar (auto kubectl port-forward):
make test-cluster                        # --no-auth → /test
make test-cluster-prod                   # production endpoint, --token-file=/tmp/user-token.txt → /mcp
```
